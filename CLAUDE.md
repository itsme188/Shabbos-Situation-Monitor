...
- Twitter uses 4-tier fallback: syndication API → BlueSky AT Protocol → Nitter RSS → Nitter HTML scraping
- BlueSky (`public.api.bsky.app`): open API, no auth. `BLUESKY_HANDLES` in config maps Twitter usernames → BlueSky handles. sentdefender registered but 0 posts as of Mar 2026
